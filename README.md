...
About accuracy: it is not being traded away for any of this. But the emulation core is
inherited, not written here, and at some point this fork will stop taking upstream's
low-level fixes automatically. So the honest promise is to keep what works and to fix what
can be fixed here - not to claim this project can maintain core accuracy at the level of
projects that specialize in exactly that.

## What is already different

On top of upstream build `20260801`: lower input lag, a memory heat map in the debugger,
