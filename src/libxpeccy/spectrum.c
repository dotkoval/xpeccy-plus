...
#include "version.h"		// generated by CMake: the name and the release bit
...
// implementing the behaviour of a much later one.
static const unsigned char* comp_version_block(void) {
	...
	if (!built) {
		...
		strncpy((char*)blk, XPRODUCT, 12);
		if (sscanf(__DATE__, "%7s %d %d", mon, &day, &year) == 3) {
			p = strstr(mons, mon);
			m = p ? ((int)(p - mons) / 3 + 1) : 1;
			blk[12] = (day & 0x1f) | ((m & 0x07) << 5);
			blk[13] = ((m >> 3) & 0x01) | (((year - 2000) & 0x3f) << 1);
			if (XBUILD_DATE[0] == 0) blk[13] |= 0x80;	// no build date: a release
		}
	}
	return blk;
}
...
// manual was written). Only cell 0 answers; the rest read as 0xFF. The picture
// here is always progressive, so VGA reads as set. The two raster bits are the
// PENT marker of a real machine - they stay at pentagon, which is the raster
// this emulator gives BaseConf, because the layout is fixed per machine.
#define MODE_VGA	0x01
#define MODE_CAPSLED	0x04
#define MODE_TAPEOUT	0x08

static unsigned char cms_modes(Computer* comp) {
	return MODE_VGA;
}
...
unsigned char cmsRd(Computer* comp) {
	...
	if (comp->cmos.adr >= 0x70) {
		switch(comp->cmos.mode) {
			...
			case 1: res = comp_version_block()[comp->cmos.adr & 0x0f]; break;	// bootloader version
			case 2: res = xt_read(comp->keyb); break; //keyReadCode(comp->keyb); break;		// read PC keyboard keycode (TODO: used here only)
			case 3: if (!(comp->cmos.adr & 0x0f)) res = cms_modes(comp); break;	// avr flags
		}
	} else {
